...
import type { InventoryItem, InventoryStatus, RestockProduct } from "../technician.types";
...
export function deriveProductStatus(
  ...
  if (!product.isActive || product.totalQuantityAvailable <= 0) {
    return "Out of Stock";
  }

  if (product.inventoryLots.some((lot) => isExpiringSoon(lot.expiry))) {
    return "Expiring Soon";
  }
