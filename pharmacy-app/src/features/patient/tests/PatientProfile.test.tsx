...
function setDirectoryReturn(dir: DirState) {
  (usePatientDirectory as unknown as vi.Mock).mockReturnValue(dir);
  return dir;
}
function setDetailsReturn(det: DetailsState) {
  (usePatientDetails as unknown as vi.Mock).mockReturnValue(det);
  return det;
}
function setRxReturn(rx: RxState) {
  (usePatientPrescriptions as unknown as vi.Mock).mockReturnValue(rx);
  return rx;
}
...
describe('PatientProfiles', () => {
  ...
  it('AddPatientModal onSave success: creates patient, refreshes list, selects new patient, closes', async () => {
    ...
    (patientApi.createPatient as vi.Mock).mockResolvedValue({ patientId: 'p-2' });
    const refreshed = [{ id: 'p-2', fullName: 'P 2' }];
    (patientApi.searchPatients as vi.Mock).mockResolvedValue(refreshed);
  ...
  it('AddPatientModal onSave failure: shows toast error and modal stays open', async () => {
    ...
    (patientApi.createPatient as vi.Mock).mockRejectedValue(new Error('bad'));
  ...
  it('UpdatePatientModal onSave: sets selected, refreshes list and closes', async () => {
    ...
    (patientApi.searchPatients as vi.Mock).mockResolvedValue([{ id: 'p-1', fullName: 'Updated P' }]);
