...
type UsePatientSearchReturn = {
  query: string;
  results: Array<{ id: string; fullName: string; phone: string }>;
  loading: boolean;
  error: string | null;
  showResults: boolean;
  onQueryChange: (v: string) => void;
  selectPatient: (p: any, cb: (p: any) => any) => void;
  openResults: () => void;
};
...
function setHookReturn(partial: Partial<UsePatientSearchReturn> = {}) {
  ...
  (usePatientSearch as unknown as vi.Mock).mockReturnValue({ ...base, ...partial });
...
describe('PatientStep', () => {
  ...
  const calls = (onQueryChange as unknown as vi.Mock).mock.calls.map((c) => c[0]);
  ...
  it('renders results and clicking a row calls selectPatient with onChange', async () => {
    ...
    const onChange = vi.fn();
    const selectPatient = vi.fn((p: any, cb: (x: any) => any) => cb(p));
  ...
  describe('Selected Patient Card & age calculation', () => {
    ...
    it('renders selected patient card with computed age (birthday later in year => age--)', () => {
      ...
      expect(screen.getByText('25y / M')).toBeInTheDocument();
    ...
    it('shows em dash when DOB is empty or invalid and hides allergies when none', () => {
      ...
      expect(screen.getByText('—y / F')).toBeInTheDocument();
