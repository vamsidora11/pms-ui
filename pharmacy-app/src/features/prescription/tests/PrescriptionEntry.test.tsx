...
import { render, screen, fireEvent } from "@testing-library/react";
import type { DoctorDetails, MedicationDraft, PatientDetails } from "../types/models";
...
vi.mock("@components/common/Stepper/Stepper", () => ({
  default: ({ currentStep, steps }: { currentStep: number; steps: Step[] }) => (
    <div data-testid="stepper">
...
vi.mock("@prescription/steps/MedicationStep", () => ({
  default: ({
    ...
    onChange: (meds: MedicationDraft[]) => void;
  }) => (
    <div data-testid="medication-step">
...
const mockUsePrescriptionEntry = usePrescriptionEntry as unknown as Mock;
...
type HookReturn = {
  ...
  goNext: () => void;
  goPrev: () => void;
  handlePatientSelected: (id: string) => void;
  handleDoctorChange: (doctor: DoctorDetails) => void;
  handleMedicationsChange: (meds: MedicationDraft[]) => void;
  handleSubmit: () => void;
};
...
describe("PrescriptionEntry", () => {
  it("Step 1: renders PatientStep, shows disabled Previous and Next when isNextDisabled=true; patient change triggers handler", () => {
    ...
    mockUsePrescriptionEntry.mockReturnValue(
      makeHookReturn({
        ...
        goPrev,
        handlePatientSelected,
        draft: { patient, doctor: { id: "", name: "" }, medications: [], notes: "" },
      })
    );
  ...
  it("Step 2: renders DoctorStep; Previous enabled; doctor change triggers handleDoctorChange", () => {
    ...
    const doctor: DoctorDetails = { id: "doc-0", name: "Dr. Seed" };
    mockUsePrescriptionEntry.mockReturnValue(
      makeHookReturn({
        currentStep: 2,
        isNextDisabled: false,
  ...
  it("Step 3: renders MedicationStep; meds change triggers handleMedicationsChange", () => {
    ...
    const doctor: DoctorDetails = { id: "doc-001", name: "Dr. Test" };
    mockUsePrescriptionEntry.mockReturnValue(
      makeHookReturn({
        currentStep: 3,
        isNextDisabled: false,
    ...
    expect(handleMedicationsChange).toHaveBeenCalledWith(
      expect.arrayContaining([expect.objectContaining({ drugId: "m-001" })])
    );
  });
  ...
  it("Step 4: renders ReviewStep; hides Next; Previous enabled; submit triggers handleSubmit", () => {
    ...
    const doctor: DoctorDetails = { id: "doc-001", name: "Dr. Test" };
    const medications: MedicationDraft[] = [
      {
        drugId: "m-001",
        drugName: "Med A",
        strength: "500mg",
