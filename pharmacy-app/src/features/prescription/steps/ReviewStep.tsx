...
export default function ReviewStep({
  ...
  const hasErrors = !patient || !doctor.id || !doctor.name || incompleteMeds.length > 0;

  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
      <div className="p-6 border-b border-gray-100">
        <h2 className="text-xl font-semibold text-gray-900 mb-1">Review Prescription</h2>
      ...
      <div className="p-6 space-y-6">
        ...
        {hasErrors && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <h3 className="text-red-800 font-semibold mb-2">⚠️ Cannot Submit - Issues Found:</h3>
            <ul className="text-red-700 space-y-1 list-disc list-inside text-sm">
              ...
              {incompleteMeds.length > 0 && (
                <li>{incompleteMeds.length} medication(s) missing drug selection</li>
              )}
            </ul>
          </div>
        )}
        ...
        {/* PATIENT */}
        <section>
          <h3 className="text-base font-semibold text-gray-900 mb-3">Patient Information</h3>
          {patient ? (
        ...
        {/* DOCTOR */}
        <section>
          <h3 className="text-base font-semibold text-gray-900 mb-3">Prescribing Doctor</h3>
          <div className="bg-gray-50 rounded-lg p-4 grid grid-cols-2 gap-4">
            <div>
              <div className="text-xs text-gray-600 mb-1">Doctor ID</div>
              <div className="text-sm font-medium text-gray-900">{doctor.id || "(Not set)"}</div>
            </div>
            <div>
            ...
            {medications.map((m, idx) => (
              ...
              >
                ...
                <div className="grid grid-cols-4 gap-4 mb-3">
                  <div>
                    <div className={`text-xs mb-1 ${m.drugId ? 'text-blue-600' : 'text-red-600'}`}>
                      Drug Name
                    </div>
                    <div className={`text-sm font-medium ${m.drugId ? 'text-blue-900' : 'text-red-900'}`}>
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  {m.drugId && (
                    <div>
                      <div className="text-xs mb-1 text-blue-600">Inventory ID</div>
                      <div className="text-xs font-mono text-blue-900">{m.drugId}</div>
                    </div>
                  )}
                </div>
                ...
                {m.instructions && (
                  <div className={`mt-3 pt-3 border-t ${
                    m.drugId ? 'border-blue-200' : 'border-red-200'
                  }`}>
                    <div className={`text-xs mb-1 ${m.drugId ? 'text-blue-600' : 'text-red-600'}`}>
                      Instructions
                    </div>
                    <div className={`text-sm ${m.drugId ? 'text-blue-900' : 'text-red-900'}`}>
                      {m.instructions}
                    </div>
                  </div>
                )}
