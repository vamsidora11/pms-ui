...
//     );
//      console.log("Rows after commit:", updated);
//   };
...
//         ? {
//             ...r,
//             drugId: item.productId, // now guaranteed to set correctly
//             drugName: item.name,
...
//     // Commit immediately using updatedRows
//     commit(updatedRows);
//     return updatedRows;
//   });
...
//       setRows((prev) =>
//         prev.map((r) =>
//           r.uid === row.uid ? { ...r, isSearching: true } : r
//         )
//       );
//     }, 0);
//   }
...
export default function MedicationStep({
  ...
  const updateRows = (updater: (prev: MedicationRow[]) => MedicationRow[]) => {
    setRows((prev) => {
      const updated = updater(prev);
      onChange(updated.map(({ uid, isSearching, ...rest }) => rest));
      return updated;
    });
  };

  const addRow = () => updateRows((prev) => [...prev, createRow()]);
  ...
  const updateField = (uid: string, field: keyof MedicationDraft, value: string | number) => {
    updateRows((prev) => prev.map((r) => (r.uid === uid ? { ...r, [field]: value } : r)));
  };

  const startSearchMode = (uid: string) => {
  ...
  // Debounced inventory search for rows that are currently searching
  useEffect(() => {
    ...
    searchingUids.forEach((uid) => {
      ...
      (async () => {
        try {
          ...
          if (reqIdByUidRef.current[uid] !== requestId) return; // stale
            ...
            <div className="relative mb-4">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  readOnly={!row.isSearching}
                  onChange={(e) => setSearchText((s) => ({ ...s, [row.uid]: e.target.value }))}
                  onClick={() => {
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input
                  ...
                  min={1}
                  value={row.durationDays}
                  onChange={(e) => updateField(row.uid, "durationDays", Number(e.target.value))}
                  className="w-full border px-3 py-2 rounded-lg"
                />
              </div>
